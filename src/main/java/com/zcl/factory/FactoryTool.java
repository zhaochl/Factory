...
public class FactoryTool {
	public static String typeToType(String typeName) {
		...
		if (typeName.contains("float") || typeName.contains("numeric")
				|| typeName.contains("double") || typeName.contains("decimal")) {
			return "double";
		}
	...
	public static String removePrefix(String tb_tableName,
			int prefixStartIndex, int prefixEndIndex) {
		return tb_tableName.substring(prefixEndIndex, tb_tableName.length());
	}
	
	public static boolean tryToMakdir(String tbName) {
		boolean result = false;
		try {
			String filename = "src//" + tbName;
			...
			if(f.exists() && f.isDirectory()){
				...
				result = f.mkdir();
				System.out.print("mkdir "+filename+" success.");
			}
	...
	public static boolean tryToMakdir(String tbName,String prefix) {
		boolean result = false;
		try {
			//this.TbNamePHP = tb.getTbname().substring(tb.getTbname().indexOf(prefix));
			String t = tbName.substring(tbName.indexOf(prefix)+prefix.length());
			//System.out.println("tname - "+t);
			String filename = "src//" + UpperCaseFirstLetter(t);
			File f = new File(filename);
			if(f.exists() && f.isDirectory()){
				result = true;
				System.out.print("dir exists.");
